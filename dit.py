...
class DiT(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_emotions=8,  # 0:neutral 1:friendly 2:sad 3:dramatic 4:happy 5:regret 6:question 7:emphatic
    ):
        super().__init__()

        self.time_embed = TimestepEmbedding(dim)

        # Emotion conditioning: num_emotions classes + 1 null class (for CFG drop)
        self.num_emotions = num_emotions
        self.emotion_embed = nn.Embedding(num_emotions + 1, dim)
        nn.init.normal_(self.emotion_embed.weight, mean=0.0, std=0.02)
        if text_dim is None:
            text_dim = mel_dim
    ...
    def forward(
        ...
        cache: bool = False,
        emotion_id: "int['b'] | None" = None,  # emotion class IDs per sample
        drop_emotion: bool = False,  # drop emotion for CFG training
    ):
        ...
        # Emotion conditioning — injected additively into t (same mechanism as timestep)
        # During training emotion_id is ALWAYS provided (real class or null-drop via drop_emotion).
        # At inference without explicit emotion_id, default to null class so the model stays
        # in-distribution (it never saw bare t with no emotion signal during training).
        if emotion_id is None:
            emotion_id = torch.full((batch,), self.num_emotions, device=x.device, dtype=torch.long)

        if cfg_infer:
            # Cond branch: use requested emotion; Uncond branch: null emotion (not bare t)
            t_with_emo = t + self.emotion_embed(emotion_id.to(x.device))
            null_id    = torch.full((batch,), self.num_emotions, device=x.device, dtype=torch.long)
            t_no_emo   = t + self.emotion_embed(null_id)
        elif drop_emotion:
            # Training CFG drop: replace with null emotion
            null_id = torch.full_like(emotion_id, self.num_emotions).to(x.device)
            t = t + self.emotion_embed(null_id)
        else:
            t = t + self.emotion_embed(emotion_id.to(x.device))
        ...
        if cfg_infer:  # pack cond & uncond forward: b n d -> 2b n d
            ...
            t = torch.cat((t_with_emo, t_no_emo), dim=0)
